...
def main():
    ...
    # Fetch relevant parameters to make the quadcopter hover and spin CCW in place
    quadcopter_body_id = robot.find_bodies("body")[0]
    ...
    while simulation_app.is_running():
        ...
        if count % 2000 == 0:
            ...
            # reset command
            print(">>>>>>>> Reset!")
        # apply action to the robot (make the robot float and spin CCW in place)
        robot.write_root_velocity_to_sim(robot.data.default_root_state[:, 7:])
        forces = torch.zeros(robot.num_instances, 1, 3, device=sim.device)
        torques = torch.zeros_like(forces)
        forces[..., 2] = robot_mass * gravity
        torques[..., 2] = 0.05 # Spin CCW
        robot.set_external_force_and_torque(forces, torques, body_ids=quadcopter_body_id[0])
